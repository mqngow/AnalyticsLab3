...
# Independent Business Metric: Assuming more students graduating equates to
# more popularity to the college, which factors can be improved to raise the
# amount of students who graduate.
college_url = (
    "https://raw.githubusercontent.com/UVADS/DS-3021/refs/heads/main/"
...
# Independent Business Metric: Assuming people are constantly searching for
# jobs, we can search for what indicates someone getting placed in a job to
...
# %%
# Create target variable, try to separate to a high and low graduation rates
# to classify schools.
...
college_prev = (
    college.high_grad_4y.value_counts()[1] / len(college.high_grad_4y)
)
...
college = college.drop(
    ["index", "unitid", "chronname", "city", "site", "nicknames", "similar"],
    axis=1
)
...
college.state = (
    college.state.apply(lambda x: x if x in top_states else "Other")
).astype("category")
...
train_prev = train_counts[1] / train_counts.sum()
print(f"Training prevalence: {train_counts[1]}/{train_counts.sum()} = "
      f"{train_prev:.2%}")
...
tune_prev = tune_counts[1] / tune_counts.sum()
print(f"Tuning prevalence: {tune_counts[1]}/{tune_counts.sum()} = "
      f"{tune_prev:.2%}")
...
test_prev = test_counts[1] / test_counts.sum()
print(f"Test prevalence: {test_counts[1]}/{test_counts.sum()} = "
      f"{test_prev:.2%}")
...
# %%
# Looking at what is important for the problem, I am removing the sl_no,
# status, gender and salary as they aren't really indicators of someone
# finding employment. Also, I wanted to filter in data purely from the
# academic field, and by including gender it isn't really relevant.
...
What do your instincts tell you about the data. Can it address your problem,
what areas/items are you worried about?
"""

# College Data Set:
# I think this data can address my problem, however it doesn't track
# individual students but different institutions. Because of this, there may
# be a lot of noise within training.
# Also, with missing information from different states, information may be
# skewed by how well data is recorded in different parts of the U.S.
# The other graduation data may affect the 4 year graduation target by
# fitting close to the target.

# Job Data Set:
# The job data set tells us information that we can definitely use. I did
# drop salary as it doesn't affect our goal, job placement.
# Although, the data set is relatively small so there is a worry of the model
# overfitting.
